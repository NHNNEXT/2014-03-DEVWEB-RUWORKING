...
<div id="pageContainer">
	<div id="ns-index">
		<div class="card type1">
			${request.politicianName[0]} <span>공약 이행률 순위</span> <a
				href="/ranking.ruw"><span class="more">더보기</span></a>
			<div class="politicianList">
				<ul>
					<c:forEach var="each" items="${Top5List}" varStatus="status">
						<li><a href="/viewDetail.ruw?pid=${each.politicianId}">
								<div class="photo">
									<span>${status.count}</span> <img src="${each.imgUrl}"
										alt="${each.name} 의원 사진">
								</div>
								<div class="info">
									<span class="name">${each.name}</span> <span class="party">${each.party}</span>
									<span class="line"></span> <span class="district">${each.local}</span>
									<span class="now">이행률</span> <span class="percentage">${each.promiseFulfillment}</span>
		...
		<div class="secondfloor">
			<div class="boxTypeA">
				...
				<div class="card type2">
					...
					<div class="loading">
						로딩중<br>위치 정보 사용에<br>동의해 주세요
					</div>
				</div>

			</div>
			<div class="boxTypeB">
				<span class="title">19대<br>국회의원 평균 공약 이행률
				</span>
			...
			<div class="boxTypeC">
				<span class="title">19대<br>국회의원 임기일 D-DAY
				</span>
				<div class="card type2 dday">
					<div class="number">${DDay}</div>
				</div>
			</div>
			...
			<div class="graph-tip">[Tip] 그래프의 각 항목 위에 마우스를 올리시면 자세한 정보를 확인할
				수 있습니다</div>
		</div>
